...
#weather['半月'] = weather.loc[(weather['日期']<=15)&
...
train_data,test_data = get_train_and_test_data(weather, rice)
...
clf = GridSearchCV(xgb_model,
                   {'max_depth': [2,3,4,5,6,7],
                    'n_estimators': [50,100,120,130,150,200]}, verbose=2 ,n_jobs=44, cv=10)
...
actuals = y
print(mean_squared_error(actuals, predictions)/(2*81))
...
result = result.rename(columns={0:'2018年早稻'})
rice_new = rice.merge(result, on=['区县id'])
print('diff')
print(np.mean(rice_new['2018年早稻']- rice_new['2017年早稻']))
